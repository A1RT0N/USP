...
void rearranjar_heap(int v[], int i, int tamanho_da_heap){
    ...
    if(maior != i) {
        aux = v[i];
        v[i] = v[maior];
        v[maior] = aux;  // swap do maior com i (obv pq vc vai trocar o filho maior com o pai menor)
...
int trocar_raiz(int v[], int tamanho_do_vetor){ // trocar a raiz com o último elemento do vetor
    ...
    v[tamanho_do_vetor-1] = aux;  // Swap entre 0 e o último
    printar_heap(v, tamanho_do_vetor-1, v[tamanho_do_vetor-1]);
    return aux;
}
...
int main(){
    ...
    printar_heap(vetor_ordenador, a, 0);
