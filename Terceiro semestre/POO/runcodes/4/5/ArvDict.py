class ArvBin:
    ...
    def set_node(self, no, novo_valor):
        self.vetor[no] = novo_valor
        
    # Não faz sentido get_node aqui pq já tem no python 
    
    def node_right(self, no):
        return self.vetor[no][1]
    
    def node_left(self, no):
        return self.vetor[no][0]
    
    def count_nodes(self, i):
        if self.vetor[i][0] == self.vetor[i][1] and self.vetor[i][0] == None:
            return 0
        
        a = self.count_nodes(self.node_left(i))
        b = self.count_nodes(self.node_right(i))
    ...
    def insert(self, valor):
        if(not self.find(valor=valor)):
            self.vetor[valor] = (None, None)
            return True
    ...
    # def find(self, valor, index_inicio = 0):
    #     for chave, tuple in self.vetor.items():
    #         if chave == valor:
    #             return True
    #         else:
    #             if chave < valor:
...
#     protected int busca_aux(int i, T v){  
...
#     private void insert_aux(int index, T v){
...
#         ArrayList<T> bfs = bfs(index_busca, true); 
...
#     protected ArrayList<T> bfs(int index, boolean flag_remover_nos) {
