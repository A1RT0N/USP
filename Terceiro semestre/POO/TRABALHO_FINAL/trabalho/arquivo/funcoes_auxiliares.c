...
void ArquivoToList(FILE *fp, Lista *lista){ 
    ...
    */
    ...
    while (ByteOffset != -1){ //enquanto ainda tiver registros removidos
        ...
        // ler_lixo(fp, 1); //pular para ver se ele está removido ou não

        // int tamanho; ler_inteiro(fp, &tamanho); //ler o tamanho do registro
        // long int proxByteOffset; ler_long_int(fp, &proxByteOffset); //ler para onde ele aponta
        // int id; ler_inteiro(fp, &id); //ler o id do registro
